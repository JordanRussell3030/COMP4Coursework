...
User & Login Information & The user manually inputs their username using a keyboard & Username & Certification Path Verification Algorithm \\ \hline
User & Login Information & The user manually inputs their password using a keyboard & Password & Certification Path Verification Algorithm \\ \hline
Administrator & Name & Is added manually when a class is being set up & First Name & Database \\ \hline
Administrator & Name & Is added manually when a class is being set up & Surname & Database \\ \hline
User, Administrator & Task or Activity & Each task name is hard-coded & Trigonometry Test 1 & Administrator, Database, User \\ \hline
Program & Type & Type of task is hard-coded & Angles & Administrator, Database, User \\ \hline
Administrator & Created & The date the administrator sets the task is automatically saved when set & 14/10/15 & Database \\ \hline
User & Completed & The date when the student completes the task is saved automatically & Yes & Administrator, Database \\ \hline
User & Start Date & The date when the student first attempts the task is saved automatically & 16/10/15 & Administrator, Database \\ \hline
Administrator & Due & The date when the task is due is set manually by the administrator & 21/10/15 & Database, User \\ \hline
Administrator & Feedback & Text typed by the administrator and saved manually & Good work & Database, User \\ \hline
\end{tabular}
\end{center}
...
\hline
\textbf{Source} & \textbf{Data} & \textbf{Process} & \textbf{Example Data} & \textbf{Destination} \\ \hline
System & Level & Level is hard-coded & 8 & Database, User \\ \hline
System & Topic & Topic is hard-coded & Tan Rule & Database, User \\ \hline
System & Task Name & Task name is hard-coded & Tan Rule Practice & Database, User \\ \hline
System & Number of Tries & Number of tries is automatically recorded, incrementing the total each time the task is opened and marked & 3 &Administrator, Database \\ \hline
User & Last Tried & The date the student most recently attempted the task is automatically saved after each attempt and the previous data is over-written & 17/10/15 & Administrator, Database \\ \hline
System & Rating & Depending on the student's score, a coloured face (red, amber or green) is automatically saved when a task is submitted & Amber face graphic & Administrator, Database \\ \hline
User & Percentage & calculated from the amount of marks a student gets for each task & 85\% & Administrator, Database \\ \hline
User & Question Percentage & Percentage calculated from the amount of marks awarded to the student for each individual question & 90\% & Administrator, Database \\ \hline
Administrator & Students Belong To These Classes & All students added when setting up a class are stored in a separate table in the database & John Smith & Database \\ \hline
\end{tabular}
\end{center}
...
\hline
\textbf{Source} & \textbf{Data} & \textbf{Process} & \textbf{Example Data} & \textbf{Destination} \\ \hline
Administrator & User ID & The user ID is automatically generated when a class is added to the system & 0001 & System, User
\end{tabular}
\end{center}
