...
User & Login Information & The user manually inputs their username using a keyboard & john\_smith\_b, string & Certification Path Verification Algorithm \\ \hline
User & Login Information & The user manually inputs their password using a keyboard & SFC, string & Certification Path Verification Algorithm \\ \hline
Administrator & Name & Is added manually when a class is being set up & John, string & Database \\ \hline
Administrator & Name & Is added manually when a class is being set up & Smith, string & Database \\ \hline
...
Program & Type & Type of task is hard-coded & Angles, string & Administrator, Database, User \\ \hline
...
System & Number of Tries & Number of tries is automatically recorded, incrementing the total each time the task is opened and marked & 3, integer & Administrator, Database \\ \hline
User & Last Tried & The date the student most recently attempted the task is automatically saved after each attempt and the previous data is over-written & 17/10/15, date & Administrator, Database \\ \hline
System & Rating & Depending on the student's score, a coloured face (red, amber or green) is automatically saved when a task is submitted & Amber face graphic, graphic & Administrator, Database \\ \hline
User & Percentage & calculated from the amount of marks a student gets for each task & 85\%, real & Administrator, Database \\ \hline
User & Question Percentage & Percentage calculated from the amount of marks awarded to the student for each individual question & 90\%, real & Administrator, Database \\ \hline
...
User & Overall percentage score & The percentage of the questions in a task which the user got correct is recorded automatically upon submission by the system & 70\%, real & Administrator, Database \\ \hline
User & Individual question percentage score & The percentage score for each individual question is recorded automatically upon task submission & 50\%, real & Administrator, Database \\ \hline
System & Rating & A red, amber or green face is recorded for each submitted assignment to represent whether or not the user did well enough & Green face graphic, graphic & Administrator, Database \\ \hline
...
\hline
\textbf{Source} & \textbf{Data} & \textbf{Process} & \textbf{Example Data} & \textbf{Destination} \\ \hline
Administrator & See after class & The administrator can manually set parameters so that the system knows to inform a student if they need to see the teacher after their next lesson e.g Below 50\% & Yes, boolean & User, Database \\ \hline
Administrator & Time of appointment & The time for a meeting with the user if they need to see the teacher, input manually be the teacher & 11:00AM, time & Database, User \\ \hline
\end{tabular}
\end{center}
